...
def backtracking_linesearch(x,f,f_g,p,alpha0=1.,tau=0.99, c=0.5, maxit=2E9):
    alpha=alpha0; j=0
    m=f_g(x).T@p
    while(f(x+alpha*p)>= f(x)+c*alpha*m and alpha>1E-17):
        j+=1
        alpha*=tau
    #print("alpha=",alpha)
    return(alpha)
...
def Newton(f, f_g, f_h, x0, truemin, maxit=2E9):
    ...
    while(it<maxit and np.linalg.norm(p)>1E-8 ):
        ...
        step=backtracking_linesearch(x,f,f_g,p)*p
        x+= step
...
def steepest_descent(f, f_g, x0, truemin, alpha0, maxit=1E2):
    x=x0;it=0; p=10.;  convergence=[]; res=[]
...
print("x=", x1)
x2, convergence2, res2=Newton(f4("0"), f4("1"),f4("2"), np.array([1E-6, 1E-6]), np.array([4.55E-8,4.55E-8]))
print("x=", x2)
